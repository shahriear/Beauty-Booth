...
// import { getBogoProducts } from '@/services/productService';

// const Slider = dynamic(() => import('react-slick'), { ssr: false });

// export default function BuyOneGetOne() {
//   const [mounted, setMounted] = useState(false);
//   const { data: products, loading } = useApi(() => getBogoProducts(6), []);
...
//         <img src="/images/NewArrival.webp" alt="BOGO Madness" className="w-full object-cover" />
//       </div>
//     </section>
//   );
// }
...
/* Cache */
let bogoProductsCache = null;

export default function BuyOneGetOne() {
  const { data } = useApi(async () => {
    if (bogoProductsCache) return bogoProductsCache;

    const res = await getBogoProducts(6);
    bogoProductsCache = res;
    return res;
  }, []);

  const products = data || bogoProductsCache || [];
