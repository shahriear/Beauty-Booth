...
// import { getFreeDeliveryOffers } from '@/services/offerService';

// const Slider = dynamic(() => import('react-slick'), { ssr: false });

// export default function FreeDeliveryAvailable() {
//   const [mounted, setMounted] = useState(false);
//   const { data: items, loading } = useApi(() => getFreeDeliveryOffers(), []);
...
export default function FreeDeliveryAvailable() {
  const { data: items, loading } = useApi(async () => {
    if (freeDeliveryCache) return freeDeliveryCache;

    const res = await getFreeDeliveryOffers();
    freeDeliveryCache = res;
